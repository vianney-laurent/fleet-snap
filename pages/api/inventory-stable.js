...
async function handler(req, res) {
  ...
  try {
    ...
    // 1. Parser le form-data avec limites augmentées
    ...
    // Normaliser les champs (formidable peut retourner des tableaux)
    fields = {};
    for (const [key, value] of Object.entries(rawFields)) {
      fields[key] = Array.isArray(value) ? value[0] : value;
    }

    // Vérifier que des fichiers ont été reçus
    if (!files || !files.photos) {
      logger.warn('Aucun fichier photo reçu', { 
        hasFiles: !!files,
    ...
    // Vérifier que la zone est fournie
    if (!fields.zone) {
      logger.warn('Zone manquante', { fields: Object.keys(fields) });
    ...
    if (!token || token.length < 10) {
      logger.warn('Token malformé', { tokenLength: token?.length });
    ...
    for (const file of uploadedFiles) {
      try {
        ...
        const fileName = file.originalFilename || file.name || 'unknown.jpg';
        const sanitizedFilename = fileName.replace(/[^a-zA-Z0-9.-]/g, '_');
        const filePath = `inventories/${user_id}/${Date.now()}_${sanitizedFilename}`;
    ...
    if (records.length === 0) {
      return res.status(400).json({ error: 'Aucun fichier traité avec succès' });
    }
